...
function App() {
  ...
  const handleSaveCalculation = () => {
    const dateSavedStr = new Date().toLocaleDateString('en-IN', {
      day: '2-digit',
      month: 'short',
      year: '2-digit',
      hour: '2-digit',
    ...
    let receiptText = `Milk Calculation Summary\n`;
    receiptText += `------------------------\n`;
    receiptText += `Month: ${monthName} ${year}\n`;
    receiptText += `Total Liters: ${calculations.totalLiters.toFixed(2)} L\n`;
    receiptText += `Cost per Liter: ₹${baseCost}\n`;
    receiptText += `Total Cost: ₹${calculations.totalCost.toFixed(2)}\n`;
    receiptText += `Saved On: ${dateSavedStr}\n\n`;
    
    receiptText += `Daily Breakdown:\n`;
    receiptText += `------------------------\n`;
    for (let d = 1; d <= daysInMonth; d++) {
      const qty = getQuantityForDay(d);
      const dayStr = String(d).padStart(2, '0');
      receiptText += `${dayStr} ${monthName}: ${qty > 0 ? qty + ' L' : '-'}\n`;
    }
    receiptText += `------------------------\nGenerated by Pure Calc`;
    ...
    link.href = url;
    link.download = `Milk_Calculation_${monthName}_${year}.txt`;
